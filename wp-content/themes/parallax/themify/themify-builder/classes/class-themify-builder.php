...
class Themify_Builder {
	...
	function init() {
		...
		// New Feature parallax element scrolling
...
?>
	...
	private function do_color_overlay($styling) {
		...
		if( ! isset( $styling['cover_color-type'] ) ||  $styling['cover_color-type'] == 'color'  ) {
			if (empty($styling['cover_color']) && empty($styling['cover_color_hover'])) {
				return false;
			}
		} elseif(empty($styling['cover_gradient-css']) && empty($styling['cover_gradient_hover-css']) ) {
			return false;
		}
		...
		} elseif(!empty( $styling['cover_color-type'] ) &&  !empty($styling['cover_gradient-css'])) {
			// using gradient
			$atts['data-type']='gradient';
		}
		if(!isset( $styling['cover_color_hover-type'] ) || $styling['cover_color_hover-type'] == 'hover_color'){
			if(!empty($styling['cover_color_hover'])){
				$atts['data-hover-color'] = $this->get_rgba_color($styling['cover_color_hover']);
			}
		}
		elseif(isset( $styling['cover_color_hover-type'] ) && !empty($styling['cover_gradient_hover-css'])){
				...
				<?php endif; ?>
												...
												if (isset($col['modules']) && count($col['modules']) > 0) {
													foreach ($col['modules'] as $modules => $mod) {
														...
														clearfix %s"%s %s %s>', esc_attr($print_sub_row_classes), $sub_row_attr, $sub_row_column_equal_height_data, $sub_row_column_alignment_data);
															?>
	...
	*/
	...
	public function parallax_elements_props( $props, $fields_args, $mod_name, $module_ID ) {
		...
		if ( isset( $fields_args['custom_parallax_scroll_delay'] ) && '' != $fields_args['custom_parallax_scroll_delay'] ) 
			$props['data-parallax-element-delay'] = $fields_args['custom_parallax_scroll_delay'];

		if ( isset( $fields_args['custom_parallax_scroll_start'] ) && '' != $fields_args['custom_parallax_scroll_start'] ) 
			$props['data-parallax-element-start'] = $fields_args['custom_parallax_scroll_start'];

		if ( isset( $fields_args['custom_parallax_scroll_end'] ) && '' != $fields_args['custom_parallax_scroll_end'] ) 
			$props['data-parallax-element-end'] = $fields_args['custom_parallax_scroll_end'];
