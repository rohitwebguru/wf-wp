...
(function($, _, window, document, undefined) {
	...
	ThemifyPageBuilder = {
		...
		_gridMenuClicked: function(event) {
			...
			var $grid = $(this).closest('.themify_builder_grid_list');
			if (set[0] == '-full') {
		...
		responsiveFrame: {
			...
			init: function() {
				...
				$frame.find('body').addClass('themify_builder_active themify_builder_responsive_frame_body');
				...
				$frame.on(eventToUse, '.themify_builder_module_front', function(event) {
					...
					.on('click', '.themify_builder_style_row', function(event) {
						event.preventDefault();
						var path = ThemifyBuilderCommon.getDomPath(this),
							selector = path.join(' > ');
						$(selector).trigger('click');
					})
					.on('click', '.themify_builder_option_column', function(event) {
						event.preventDefault();
						var path = ThemifyBuilderCommon.getDomPath(this),
							selector = path.join(' > ');
						$(selector).trigger('click');
					})
					.on('click', '.js--themify_builder_module_styling', function(event) {
						event.preventDefault();
						var path = ThemifyBuilderCommon.getDomPath(this),
							selector = path.join(' > ');
						$(selector).trigger('click');
	...
	ThemifyLiveStyling = (function($, jss) {
		...
		ThemifyLiveStyling.prototype.addStyleDate = function($module,$rules) {
			...
			if(!this.isModuleExists[$module]){ console.log($module)
		...
		ThemifyLiveStyling.prototype.bindTextInputs = function() {
			...
			$(document).on('tfb.live_styling.after_init',function(){
				setTimeout(function(){
					var $border_input = $(self.style_tab +' input[type="text"].style_border');
					$border_input.each(function(){
						$(this).trigger('keyup');
						$(this).closest('.themify_builder_field').find('select').trigger('change');
					});
				},900);
					...
					if ($(this).hasClass('font-family-select')) {
						...
						if ($(this).closest('.themify_builder_input').find('.thumb_preview').length > 0) {
							...
							return false;
                                        ...
                                        else if($val!=='none' && $(this).is('select') && $(this).find('[value="dashed"]').length>0  && !parseInt($(this).closest('.themify_builder_input').find('.style_border').val())){
                                            return false;
                                        }
					self.setLiveStyle({
						[$data.prop]: $val
