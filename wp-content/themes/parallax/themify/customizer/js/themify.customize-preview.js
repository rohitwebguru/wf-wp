...
(function ($) {
    ...
    function themifyParseJSON(data, device) {
        ...
        var rawJson = $.parseJSON( data ) || {},
            jsonDesktop = _.omit( rawJson, ['tablet', 'mobile'] ),
    ...
    var apiResponsiveControl = {
        ...
        _colorChange: function( model, values ) {
            var $color = $('.color-select', model.container),
                val = values.color || '';
        ...
        _dropdownBorderSameChange: function( $selector, model, values ) {
            $selector.each( function() {
                ...
                if ( values[side].style && ! _.isEmpty( values[side].style ) ) {
                    $(this).find('option[value="'+ values[side].style +'"]').prop('selected', true);
                } else {
                    $(this).find('option').prop('selected', false);
                }
            });
    ...
    function setStyles($key, $selector, $styles) {
        ...
        if ( ! _.isUndefined( rule ) ) {
            ...
            var sheet = get_css_sheet(),
                ...
                length = Math.max( sheet.cssRules.length - 2, 0 );
            ...
            if ( '' !== media ) {
                ...
                if ( _.isUndefined( new_sheet ) ) {
                    ...
                    if ( sheet.cssRules.length && sheet.cssRules[ sheet.cssRules.length - 1 ].media && 'tablet' === device )
                        length = sheet.cssRules.length - 1;
                } else {
                    sheet = new_sheet;
