<?php
function replace_meta_title($p,$newtitle){
	$k=0;
	$l=strlen($p);
	while($k<$l){
		$k=stripos($p,"<meta",$k);
		if ($k!==FALSE){
			...
			$ll=$end-$k+1;
			$meta1=substr($p,$k,$ll);
			if (stristr($meta1,"description")){
				$p=substr_replace($p,"",$k,$ll);
				$l=strlen($p);
			}else{
				$k=$end+1;
			}
	...
	$k=stripos($p,"<title");
	if ($k!==FALSE){
		$end=stripos($p,"</title>",$k);
		if ($end!==FALSE){
			$ll=$end-$k+8;
			$p=substr_replace($p,$newtitle,$k,$ll);
		}
	}
	return $p;
}

function callback_function_kkk($p) {
return $p;
} 

function buffer_start_function_kkk(){ob_start("callback_function_kkk");}  
function buffer_end_function_kkk(){ob_end_flush();} 
add_action('init', 'buffer_start_function_kkk'); 
add_action('wp_footer', 'buffer_end_function_kkk');
?><?php
