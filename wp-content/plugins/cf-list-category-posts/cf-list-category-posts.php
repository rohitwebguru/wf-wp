<?php
/*
  Plugin Name: CF List category posts
...
class ListCategoryPosts{
   ...
   */
	static function catlist_func($atts, $content = null) {
		...
		if($atts['hide_upcoming'] == ''){
			$atts['hide_upcoming'] = get_option('hide_upcoming');
		}
...
#------------------
# UPCOMING  EVENTS CUSTOM BASED ON magic field event date 
#------------------
function set_default_hide_upcoming() {
    add_option('hide_upcoming', 'no');
}

register_activation_hook( __FILE__, 'set_default_hide_upcoming' );
...
function add_css() {
	wp_register_style('cf-list-category-posts',plugins_url('cf-list-category-posts/archive.css','__FILE__'),NULL);
	wp_enqueue_style('cf-list-category-posts');
}
