...
class WBMPL_main extends WBMPL_base
{
    ...
    public function upgrade_notice()
    {
        ...
        . '<p>'.sprintf(__("It's time to show your support by upgrading your %s to %s version. Quality checked by Envato!", WBMPL_TEXTDOMAIN), '<strong>'.__('Magic Post Listing', WBMPL_TEXTDOMAIN).'</strong>', '<strong>'.__('PRO', WBMPL_TEXTDOMAIN).'</strong>').'</p>'
        . '<span>'.sprintf(__('Extend basic plugin funtionality with %s, %s, %s and %s layouts, shortcode and other great features. See %s website!', WBMPL_TEXTDOMAIN), '<strong>'.__('Caption', WBMPL_TEXTDOMAIN).'</strong>', '<strong>'.__('Ticker', WBMPL_TEXTDOMAIN).'</strong>', '<strong>'.__('Animate', WBMPL_TEXTDOMAIN).'</strong>', '<strong>'.__('Masonry', WBMPL_TEXTDOMAIN).'</strong>', '<a href="http://webilia.com/api/mpl/redirect.php?action=demo" target="_blank">'.__('demo', WBMPL_TEXTDOMAIN).'</a>').'</span>'
        . '</div>'
        . '<div class="wbmpl_upgrade_button">'
        . '<a href="http://webilia.com/api/mpl/redirect.php?action=upgrade" target="_blank"><i class="fa fa-arrow-up"></i>'.__('Upgrade for only $17', WBMPL_TEXTDOMAIN).'</a>'
        . '</div>'
        . '</div>'
        . '</div>';
    }
