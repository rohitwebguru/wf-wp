...
class WBMPL_file extends WBMPL_base
{
	...
	public static function write($file, &$buffer)
	{
		...
		{
			WBMPL_folder::create(dirname($file));
		}
	...
	public static function upload($src, $dest)
	{
		...
		$dest = WBMPL_path::clean($dest);
		$baseDir = dirname($dest);

		if (!file_exists($baseDir))
		{
			WBMPL_folder::create($baseDir);
		}

		if (is_writable($baseDir) && move_uploaded_file($src, $dest))
		{
			// Short circuit to prevent file permission errors
			if (WBMPL_path::setPermissions($dest)) $ret = true;
			else $ret = false;
		}
		else $ret = false;
...
class WBMPL_folder extends WBMPL_base
{
	...
	public static function create($path = '', $mode = 0755)
	{
		...
		if (!self::exists($parent))
		{
			...
			{
				// WBMPL_folder::create throws an error
				$nested--;
				return false;
			}
	...
	public static function delete($path)
	{
		...
		if (!empty($files))
		{
			if (WBMPL_file::delete($files) !== true)
			{
				return false;
			}
		}
		...
		foreach ($folders as $folder)
		{
			if (is_link($folder))
			{
				if (WBMPL_file::delete($folder) !== true)
				{
					return false;
				}
			}
		...
		if (is_dir($path))
		{
			...
			while ($file = readdir($dh))
			{
				if ($file != '.' && $file != '..')
				{
					...
					if (is_dir($fullpath))
					{
						if (!WBMPL_path::setPermissions($fullpath, $filemode, $foldermode))
						{
							$ret = false;
						}
					}
